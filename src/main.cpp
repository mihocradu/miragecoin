...
#if defined(NDEBUG)
# error "Miragecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf749e4bc7095375cee4942b212e4a657d2b24e2aecbb30fcc5b7268902e5594d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Miragecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Miragecoin Signed Message:\n";
...
{
    // Miragecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Miragecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Miragecoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Miragecoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Miragecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Miragecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x831fa9eae0fd1dd5690498de737c24e74968191e9f18dd4469130c462e89976d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "14.04.2020 Covid-19 spread the world. Half of the population in izolation. ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0459f27437fdaf6fd01ae25b48d8fdb4222c4f3e25fb42010cf9aa1d465821854369d2c28a9cffb47e8dddb5d8c8a6ca46cdb61b5e9e0aa78944f1cd23368a6074") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1586872079;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086168481;

        if (fTestNet)
        {
            block.nTime    = 1586872016;
            block.nNonce   = 386158915;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x4afc5d8800d32483476d79b9b8a00bf2e3f9eb1f3059233601bdae76350cfaa6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xca, 0xb8, 0xdc }; // Miragecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MiragecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MiragecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MiragecoinMiner(CWallet *pwallet)
{
    printf("MiragecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("miragecoin-miner");
    ...
    try { loop {
        ...
        printf("Running MiragecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MiragecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MiragecoinMiner, pwallet));
}
